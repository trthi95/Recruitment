...
public class VacancyDAOImp implements VacancyDAO {
	...
	public void addVacancy(String positionName, int numberOpening, String departmentName, String requirement,
		...
		vacancy.setDateClose(Timestamp.valueOf(LocalDateTime.now().plusDays(10)));
	...
	public Vacancy getVacancy(String vacancyNumber) {
		Session session = sessionFactory.getCurrentSession();
		Vacancy vacancy = (Vacancy) session.createQuery("from vacancy where vacancyNumber = '" + vacancyNumber +"'").getSingleResult();
		return vacancy;
	}

	@Transactional
	public void deleteVacancyById(int idVacancy) {
		System.out.println("bbb" +idVacancy);
		Session session = sessionFactory.getCurrentSession();
		Vacancy vacancy = (Vacancy) session.createQuery("from vacancy v where v.idVacancy = " + idVacancy +"").getSingleResult();
